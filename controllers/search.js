...
router.get("/songs", async function (req, res) {
  try {
    const response = await axios.get(
      `https://api.musixmatch.com/ws/1.1/track.search?q_track=${req.query.search}&s_track_rating=desc&f_has_lyrics=1&apikey=${API_KEY}`
    );
...
router.get("/artists", async function (req, res) {
  try {
    ...
    const response = await axios.get(
      `https://api.musixmatch.com/ws/1.1/artist.search?q_artist=${req.query.search2}&page_size=15&apikey=${API_KEY}`
    );
    res.render("lists.ejs", {
...
router.get("/artists/songs", async function (req, res) {
  try {
    ...
    const response = await axios.get(
      `https://api.musixmatch.com/ws/1.1/track.search?q_artist=${req.query.artistName}&f_artist_id=${req.query.artistId}&page_size=10&page=1&s_track_rating=desc&apikey=${API_KEY}`
    );
  ...
  try {
    ...
    if (response.data.message.body.track.has_lyrics == "1") {
      lyrics = await axios.get(
        `https://api.musixmatch.com/ws/1.1/track.lyrics.get?commontrack_id=${req.params.id}&apikey=${API_KEY}`
      );
    ...
    const img = await axios.get(
      `http://ws.audioscrobbler.com/2.0/?method=album.getinfo&api_key=${OTHER_KEY}&artist=${str}&album=${alb}&format=json`
    );
